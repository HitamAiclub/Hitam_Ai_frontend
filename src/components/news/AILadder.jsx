...
const AILadder = () => {
    ...
    const getModalityIcon = (types = [], size = 22) => {
    ...
    if (loading) return (
        <div className="p-4 md:p-8 space-y-4">
            {[1,2,3,4,5,6].map(i => (
                <div key={i} className="h-16 md:h-20 bg-gray-100 dark:bg-gray-800/30 rounded-2xl animate-pulse" />
            ))}
        </div>
    );
    ...
    return (
        <div className="space-y-6 md:space-y-10">
            ...
            {/* ─── HEADER ROW ─── */}
            <header className="flex flex-col xl:flex-row xl:items-end justify-between gap-5 md:gap-8 px-2">
                <div className="space-y-3 md:space-y-5">
                    {/* Live indicator + refresh */}
                    <div className="flex flex-wrap items-center gap-2 md:gap-3">
                        <div className="flex items-center gap-2 text-blue-600 dark:text-blue-400 font-black text-[10px] md:text-[11px] uppercase tracking-[0.15em] md:tracking-[0.2em] bg-blue-500/5 px-3 md:px-5 py-2 md:py-2.5 rounded-full border border-blue-500/10">
                        ...
                        <button
                            ...
                            className={`flex items-center gap-1.5 md:gap-2 text-[10px] font-black text-gray-400 uppercase tracking-widest bg-white dark:bg-gray-900 px-3 md:px-4 py-2 md:py-2.5 rounded-full border border-gray-200 dark:border-gray-800 hover:border-blue-500 hover:text-blue-600 transition-all ${refreshing ? 'animate-pulse' : ''}`}
                        >
                    ...
                    <div>
                        <h2 className="text-2xl sm:text-3xl md:text-5xl lg:text-6xl font-black text-gray-900 dark:text-white tracking-tighter mb-2 md:mb-3 leading-none">
                            {viewMode === "ladder" ? (
                        ...
                        <p className="text-sm md:text-base lg:text-lg text-gray-500 dark:text-gray-400 font-medium">
                            {viewMode === "ladder"
                ...
                <div className="flex flex-col gap-3 md:gap-4 w-full xl:w-auto">
                    {/* VIEW MODE TOGGLE */}
                    <div className="flex items-center gap-1.5 p-1.5 bg-gray-100 dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 w-full sm:w-fit">
                        <button
                            onClick={() => setViewMode("ladder")}
                            className={`flex items-center justify-center gap-2 px-4 sm:px-7 py-2.5 md:py-3 rounded-xl font-black text-xs sm:text-sm transition-all duration-300 whitespace-nowrap flex-1 sm:flex-none ${
                                viewMode === "ladder"
                    ...
                    <div className="relative group">
                        <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400 group-hover:text-blue-600 transition-colors" size={16} />
                        <input
                            type="text"
                            placeholder="Search models or providers…"
                            value={searchTerm}
                            onChange={e => setSearchTerm(e.target.value)}
                            className="w-full xl:w-80 bg-white/40 dark:bg-gray-900/40 backdrop-blur-xl border-2 border-gray-200 dark:border-gray-800 focus:border-blue-600 dark:focus:border-blue-600 rounded-2xl py-3 pl-10 pr-4 outline-none font-black text-sm text-gray-900 dark:text-white transition-all"
                        />
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 md:gap-2 overflow-x-auto no-scrollbar pb-1">
                        {modalities.map(cat => (
                            <button
                                ...
                                className={`flex items-center gap-1.5 md:gap-2 px-3 md:px-5 py-2 md:py-2.5 rounded-2xl font-black text-[10px] md:text-[11px] uppercase tracking-widest transition-all duration-300 border-2 whitespace-nowrap ${
            ...
            <AnimatePresence mode="wait">
                {viewMode === "ladder" ? (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: -20 }}
                        className="rounded-3xl md:rounded-[3rem] border border-gray-200 dark:border-gray-800 bg-white/40 dark:bg-gray-900/40 backdrop-blur-[16px] shadow-2xl overflow-hidden"
                    >
                        {/* ── MOBILE CARD VIEW ── */}
                        {isMobile ? (
                                ...
                                {filtered.slice(0, 50).map((model, idx) => {
                                    ...
                                    return (
                                        <motion.div
                                            ...
                                            className="flex items-center gap-3 px-4 py-4 hover:bg-blue-600/5 transition-colors"
                                        >
                                            {/* Rank */}
                                            <div className="flex flex-col items-center w-8 shrink-0">
                                                ...
                                                <ChevronUp size={12} className="text-green-500 mt-0.5" />
                                            </div>

                                            {/* Icon */}
                                            <div className="w-10 h-10 rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 flex items-center justify-center shadow shrink-0">
                                                {getModalityIcon(model.types, 18)}
                                            </div>

                                            {/* Info */}
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center gap-1.5 mb-0.5">
                                                    <h4 className="text-sm font-black text-gray-900 dark:text-white truncate leading-tight">
                                                        {model.name.split(':')[1]?.trim() || model.name}
                                                    </h4>
                                                    {model.isNew && (
                                                        <span className="text-[7px] font-black text-white bg-red-500 px-1.5 py-0.5 rounded-full animate-pulse shrink-0">NEW</span>
                                                    )}
                                                </div>
                                                <div className="flex items-center gap-1.5 mb-1.5">
                                                    <span className="text-[9px] font-black text-white bg-gray-900 dark:bg-blue-600 px-1.5 py-0.5 rounded font-mono">
                                                        {market.ticker}
                                                    </span>
                                                    <span className="text-[9px] font-bold text-gray-400 uppercase tracking-wide truncate">{model.provider}</span>
                                                </div>
                                                {/* Usage bar */}
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-[10px] font-black text-blue-600 shrink-0">{model.usage}%</span>
                                                </div>
                                            </div>
                                        </motion.div>
                                    );
                                ...
                                <table className="w-full text-left border-collapse min-w-[900px]">
                                    <thead>
                                        <tr className="border-b border-gray-200 dark:border-gray-800 bg-gray-50/60 dark:bg-gray-900/40">
                                            ...
                                            <th className="py-7 px-8 text-[10px] font-black text-gray-400 uppercase tracking-[0.25em]">Market Intel (Stock/Cap)</th>
                                            <th className="py-7 px-8 text-[10px] font-black text-gray-400 uppercase tracking-[0.25em]">Profit &amp; Growth</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <AnimatePresence mode="popLayout">
                                            {filtered.slice(0, 50).map((model, idx) => {
                                                const market = getMarketIntel(model.provider);
                                                const growth = Math.floor((model.usage / 2) + (Math.random() * 10));
                                                return (
                                                    <motion.tr
                                                        key={model.id}
                                                        layout
                                                    ...
                                                    >
                                                        ...
                                                        <td className="py-8 px-4 min-w-[260px]">
                                                            <div className="flex items-center gap-4">
                                ...
                                {perfSorted.slice(0, 50).map((model, idx) => {
                                    ...
                                    return (
                                        <motion.div
                                            ...
                                            exit={{ opacity: 0 }}
                                            transition={{ duration: 0.25, delay: idx * 0.02 }}
                                            className="flex items-center gap-3 px-4 py-4 hover:bg-emerald-600/5 transition-colors"
                                        >
                                            {/* Rank / Trophy */}
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center gap-1.5 mb-0.5">
                                                    ...
                                                    <span className={`text-[8px] font-black px-1.5 py-0.5 rounded-full border shrink-0 ${tier.color}`}>
                                                        {tier.label}
                                                    </span>
                                                </div>
                                                    ...
                                                    <div className="flex-1 h-2 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                                                        <motion.div
                                                            ...
                                                            className={`h-full rounded-full ${
                                                                ...
                                                                : score >= 55 ? 'bg-gradient-to-r from-emerald-500 to-teal-500'
                                                                : 'bg-gradient-to-r from-gray-400 to-gray-500'
                                                            }`}
                                                        />
                                                    </div>
                                                    <span className="text-[10px] font-black text-gray-900 dark:text-white shrink-0 tabular-nums">{score}<span className="text-gray-400 font-bold">/100</span></span>
                                                </div>
                                            </div>
                                        </motion.div>
                                    );
                                ...
                                <table className="w-full text-left border-collapse min-w-[900px]">
                                    <thead>
                                        <tr className="border-b border-gray-200 dark:border-gray-800 bg-gray-50/60 dark:bg-gray-900/40">
                                            ...
                                            <th className="py-7 px-8 text-[10px] font-black text-gray-400 uppercase tracking-[0.25em]">Context Window</th>
                                            <th className="py-7 px-8 text-[10px] font-black text-gray-400 uppercase tracking-[0.25em]">Value (Score/$)</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <AnimatePresence mode="popLayout">
            ...
            {/* Empty state */}
            {filtered.length === 0 && (
                <div className="text-center py-20 md:py-32 border-4 border-dashed border-gray-100 dark:border-gray-800 rounded-3xl md:rounded-[4rem]">
                    <AlertCircle size={48} className="mx-auto text-gray-200 dark:text-gray-800 mb-5" />
                    <h3 className="text-2xl md:text-3xl font-black text-gray-900 dark:text-white mb-2">No Models Found</h3>
                    <p className="text-gray-500 font-medium text-sm md:text-base">Try changing the modality filter or search term.</p>
                </div>
            )}

            {/* Footer */}
            <footer className="flex flex-col md:flex-row items-center justify-between gap-4 md:gap-6 px-4 md:px-6 py-6 md:py-10 border-t border-gray-200 dark:border-gray-800 text-[11px] font-bold text-gray-400 text-center md:text-left">
